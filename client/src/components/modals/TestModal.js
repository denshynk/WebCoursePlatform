...
const TestModal = ({ show, onHide, testId }) => {
    ...
    const handleSubmitTest = async () => {
        ...
        console.log("Submitting test:", finalAnswers);
        setTestAnswers({});
                        ...
                        <div className="d-flex flex-column">
                            <div className="mt-3">
                                <div className={`d-flex justify-content-between `}>
                                    ...
                                    <h2 className={`${timeLeft <= 60 ? 'text-danger' : ''} ${timeLeft <= 60 && timeLeft % 2 === 0 ? 'blink' : ''}`}>{formatTime(timeLeft)}</h2>
                                </div>
                                <h4 className="mt-3">
                                    ...
                                    (answer, idx) => (
                                        <div key={idx}>
                                            <input
                                                ...
                                                value={answer}
                                                checked={
                                                    testAnswers[test.questions[currentQuestion].id] === answer
                                                }
                                                onChange={() =>
                                                    handleAnswerChange(test.questions[currentQuestion].id, answer)
                                                }
                                            />
                                            <label className="ml-2" htmlFor={`${currentQuestion}-${idx}`}>
                                                {answer}
                                            </label>
                                        </div>
                                    )
